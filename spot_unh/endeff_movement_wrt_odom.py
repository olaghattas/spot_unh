...
def hello_arm(robot_name: Optional[str] = None) -> bool:
    ...
    flat_body_T_hand = tf_listener.lookup_a_tform_b(grav_aligned_body_frame_name, HAND_FRAME_NAME )
    # Make the arm pose RobotCommand
    # Build a position to move the arm to (in meters, relative to and expressed in the gravity aligned body frame).
    print(flat_body_T_hand)

    x = flat_body_T_hand.transform.translation.x + 0.1
    # x = 0
    y = flat_body_T_hand.transform.translation.y
    z = flat_body_T_hand.transform.translation.z
